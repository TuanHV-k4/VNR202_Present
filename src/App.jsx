...
function Dossier({ node, index, onNext }) {
  return <motion.article className="dossier" key={node.id} initial={{ opacity: .35, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .38 }}>
    ...
    <aside><span>{node.focusLabel || 'Ý nghĩa trong ẩn dụ'}</span><p>{node.insight}</p></aside>
  ...
  return <div className="app-shell">
    ...
    <main className="story-main" data-page={page} style={{transform:`translate3d(0, -${page * 100}svh, 0)`}}>
      ...
      <section className="workspace story-page" id="workspace">
        ...
        <div className="key-rail" role="tablist" aria-label="Các mốc thời gian" style={{gridTemplateColumns:`repeat(${allNodes.length}, minmax(0, 1fr))`}}><div className="rail-line" style={{'--rail-progress':(active+1)/allNodes.length}}/><div className="rail-key"/>{allNodes.map((node,i)=><button role="tab" aria-selected={active===i} key={node.id} className={`${active===i?'active ':''}${visited.has(i)?'visited':''}`} onClick={()=>choose(i)}><i/><span>{node.date.replace(' — ','–')}</span><small>{i+1}</small></button>)}</div>
      ...
      <section className="answer-section story-page" aria-labelledby="answer-title">
        ...
        <div className="argument-axis" aria-label="Ba tầng chứng cứ lịch sử">
          <article><div className="argument-date"><b>1939—1940</b><span>Chọn đúng hướng rèn</span></div><div><h3>Chuẩn bị chiến lược trước khi thời cơ xuất hiện</h3><p>Hội nghị Trung ương 6 đặt giải phóng dân tộc lên hàng đầu và chủ trương xây dựng mặt trận thống nhất. Hội nghị Trung ương 7 cho thấy đường lối vẫn trải qua tranh luận và cần tiếp tục hoàn thiện. Bắc Sơn và Nam Kỳ đem hướng đi ấy thử thách trong thực tiễn, để lại kinh nghiệm cùng nòng cốt lực lượng đầu tiên.</p><strong>Ổ khóa chưa xuất hiện, nhưng chiếc chìa đã được rèn qua lựa chọn, tranh luận và cả tổn thất — không phải một đường thẳng may mắn.</strong></div></article>
          <article><div className="argument-date"><b>1941—1944</b><span>Biến đường lối thành lực lượng</span></div><div><h3>Chuẩn bị có hệ thống trước khi thời cơ xuất hiện</h3><p>Hội nghị Trung ương 8 hoàn chỉnh đường lối giải phóng dân tộc; Việt Minh tập hợp lực lượng quần chúng qua các hội Cứu quốc. Cứu quốc quân, Đội Việt Nam Tuyên truyền Giải phóng quân và hệ thống căn cứ Cao Bằng – Bắc Sơn – Võ Nhai được xây dựng. Cuối năm 1943, chỉ thị “Sửa soạn khởi nghĩa” cho thấy việc chuẩn bị đã chuyển thành hành động cụ thể.</p><strong>Đến cuối năm 1944, cách mạng đã có đường lối, tổ chức, lực lượng và căn cứ; đồng thời dự đoán thời cơ giải phóng đang đến gần.</strong></div></article>
          <article className="decisive"><div className="argument-date"><b>03—08/1945</b><span>Nhận và chớp thời cơ</span></div><div><h3>Chủ động điều chỉnh và hành động đúng lúc</h3><p>Chỉ ba ngày sau cuộc đảo chính 9/3, Chỉ thị “Nhật – Pháp bắn nhau và hành động của chúng ta” xác định đúng kẻ thù và phát động cao trào kháng Nhật. Từ tháng 3 đến tháng 6, Việt Nam Giải phóng quân được thống nhất, Khu giải phóng Việt Bắc hình thành và phong trào “phá kho thóc” mở rộng cơ sở quần chúng. Khi Nhật đầu hàng, Quân lệnh số 1 được ban hành trong đêm 13/8; Quốc dân Đại hội ngày 16–17/8 hoàn thiện cơ quan lãnh đạo cách mạng.</p><strong>Tốc độ ra quyết định không phải dấu hiệu của may mắn; đó là dấu hiệu của một bộ máy đã sẵn sàng.</strong></div></article>
        </div>
