  ...
  return <div className="app-shell">
    ...
    <main className="story-main" data-page={page} style={{transform:`translate3d(0, -${page * 100}svh, 0)`}}>
      ...
      <section className="workspace story-page" id="workspace">
        ...
        <div className="workspace-grid"><aside className="archive-index fixed-index" aria-label="Danh sách hồ sơ" style={{gridTemplateRows:`repeat(${allNodes.length}, minmax(0, 1fr))`}}>{allNodes.map((node,index)=><button key={node.id} onClick={()=>choose(index)} className={active===index?'active':''}><span>{String(index+1).padStart(2,'0')}</span><p><b>{node.title}</b><small>{node.date}</small></p></button>)}</aside>
